...
_translateMemoryAddress = function(memoryAddress) {
    ...
    if(!memoryArrayArray[decodedMemory['MemoryArea']]) {
     ...
     // if bit addressing is being used and the value is a valid index
    ...
    return {address: byteEncodedMemory, bitAddressing: bitAddressing};
...
_processMemoryAreaRead = function(self,buf,rinfo) {
    ...
    // need to know if it was a bit read or a word read, look it up in our object referenced by the sid number
    if( self.readTypeQueue[sid] === true) {
    ...
    // remove the entry from the queue object
    delete self.readTypeQueue[sid];
...
FinsClient.prototype.read = function(address,regsToRead,callback) {
    ...
    var address = addressTranslation.address;
    self.readTypeQueue[self.header.SID] = addressTranslation.bitAddressing; // store whether this particular read was a bit or word type
...
FinsClient.prototype.write = function(address,dataToBeWritten,callback) {
    ...
    self.header.SID = _incrementSID(self.header.SID);
    var header = _buildHeader(self.header);
    var address = _translateMemoryAddress(address).address;
    var regsToWrite = _wordsToBytes((dataToBeWritten.length || 1));
    var command = constants.Commands.MEMORY_AREA_WRITE;
    var dataToBeWritten = bitAddressing ? _bitsToBytes(dataToBeWritten) : _wordsToBytes(dataToBeWritten);
